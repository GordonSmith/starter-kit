...
const cp_Map = new ChartPanel()
    .id("Map")
    .title("Map")
    .widget((new GMapPin() as any)
        .centerLat(38.72169760412349)
        .centerLong(-115.70617431250002)
        .zoom(4)
        .mapTypeControl(true)
...
//  Dashboard (optional) ---
export function init(placeholder: string) {
    return new Dashboard(ec)
        .target(placeholder)
        .render(w => {
            (w as Dashboard).layout({ main: { type: "split-area", orientation: "vertical", children: [{ type: "tab-area", widgets: [{ __id: "City" }], currentIndex: 0 }, { type: "tab-area", widgets: [{ __id: "Detail" }], currentIndex: 0 }, { type: "tab-area", widgets: [{ __id: "Map" }], currentIndex: 0 }], sizes: [0.08437691887973386, 0.24628687512001202, 0.6693362060002541] } });
        })
        ;
}
